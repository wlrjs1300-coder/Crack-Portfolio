...
VALID_STATUSES = {'접수완료', '처리중', '처리완료', '삭제'}
ALLOWED_TRANSITIONS = {
    '접수완료': {'접수완료', '처리중', '처리완료'},
    '처리중': {'접수완료', '처리중', '처리완료'},
    '처리완료': {'접수완료', '처리중', '처리완료'},
...
def transition_report(report, new_status, reject_reason=None, award_points=True):
    """상태를 변경하고 신고별 포인트 로그를 이용해 중복 지급/차감을 막는다."""
    normalized = normalize_status(new_status)
    current = LEGACY_STATUS_MIGRATIONS.get((report.status or '').strip(), (report.status or '').strip())
    if normalized != '삭제' and normalized not in ALLOWED_TRANSITIONS.get(current, set()):
        raise ValueError(f'{current or "현재 상태"}에서 {normalized}(으)로 바로 변경할 수 없습니다.')
    ...
    if normalized == '처리완료' and award_points:
...
def backfill_legacy_statuses():
    ...
    changed = 0
    for legacy, canonical in LEGACY_STATUS_MIGRATIONS.items():
