...
1. **AI 도로파손 자동탐지** — YOLOv8 비전 모델로 사진과 영상을 판독해 포트홀·싱크홀 유형과 신뢰도를 제공합니다. AI 결과는 접수된 신고의 처리 우선순위를 판단하는 보조 정보로 사용합니다.
...
- **크래커 포인트 시스템**: 신고 처리완료 시 포인트 적립(+20), 포인트 사용 내역(PointLog) 관리
...
### 관리자 기능
- **관리자 대시보드**: 긴급/오늘 접수/접수완료/처리중/처리완료 등 신고 현황, 우선순위 스코어링(위험도·반복제보·장기미처리 가중치)
- **신고(Incident) 관리**: 상태별/위험도별/지역별 필터링, 검색, 정렬, 단건/일괄 상태 변경, AI 재분석 트리거
- **위치 기반 그룹핑**: 반경 30m 이내의 진행 중인 중복 신고를 자동 그룹화하여 대표 신고로 통합 처리
- **회원 관리**: 권한(admin/manager/user) 변경, 계정 정지/해제, 회원별 신고 통계(처리 완료율/중복률) 상세 조회
...
| 리스크 | 방어 로직 |
|---|---|
| 어뷰징/포인트 파밍 (중복·허위 제보) | 동일 사용자 반경 30m 이내의 진행 중인 중복 신고 자동 거절 |
...
| 공무집행방해/스팸 | YOLOv8 AI 분석 결과와 중복 신고 제한을 활용해 관리자가 우선 검토할 신고를 빠르게 판단할 수 있도록 지원합니다. AI 오탐·미탐 가능성을 고려해 최종 처리 판단은 관리자가 수행합니다. |
...
AI 작업은 `ai_jobs` 테이블에 저장되어 재시작 후 복구되며, 로그인·업로드 등의 요청 제한도 DB에 기록되어 여러 서버 프로세스에서 공유됩니다. `flask retention-run`으로 보관 정책을 즉시 실행할 수 있고, 기본적으로 삭제 미디어는 7일, 위치정보는 365일 동안 보관합니다. 신고에 연결되지 않은 임시 업로드는 기본 24시간 뒤 제거됩니다.
...
- **데이터 정리 정책**: 보관 기간 설정, 삭제·임시 업로드 파일 정리, 백업 생성/검증
